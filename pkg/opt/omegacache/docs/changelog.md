...
70. **AlchemyPlus Flask + Empty Flask + Glowing Brain stackable — INVESTIGATED AND REVERTED.** Attempted to make Flask of Crystallized Intelligence (`0xffa3`), Glowing Brain (`0x2147`), and empty flask (`0x183A` = `UOBJ_EMPTY_FLASK`) stackable via `Stackable 1` in `itemdesc.cfg` and matching `0x0800` STACKABLE flag in `config/tiles.cfg`. Server-side stacking worked (items merged on deposit/drop and in the cache), but the UO client's drag-to-split UI reads the client's local `tiledata.mul` — not POL's server-side tile overrides. Without a client-distribution-level `tiledata.mul` patch, players could see stacks but couldn't split them. All changes reverted. Tracked as a future item for when client tiledata patching is part of the deploy pipeline.

71. **AlchemyPlus "You run out of flasks/bottles" bug** — Reported by Xenomath. The main potion loop in `alchemyplus.src` consumed the container via `SubtractAmount` *before* `CreateItemInContainer`, then searched for the next iteration's container and aborted with "You run out of flasks/bottles" on failure — losing the current craft's potion even though reagents + container were already consumed. Reordered to match the sibling `alchemyplus toad.src` pattern: create potion first, subtract container second, search for next-iteration container afterwards. Current craft always completes before the loop exits on out-of-container.
